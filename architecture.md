...
Fine-tuned custom models (use hosted LLM APIs, with Groq as the LLM provider for Phase 4)
...
Sub-component
Role
LLMAdapter / GroqLLMAdapter
Thin wrapper over Groq (LLM provider)
PromptTemplates
...
Implementations: GroqLLMAdapter (Groq is the LLM provider).
...
Unique areas, neighborhoods (e.g. Indiranagar, Bellandur), and parent cities
GET
/api/v1/metadata/cuisines
Unique cuisines for dropdown selection
